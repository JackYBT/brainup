...
class FBCCA():
    ...
    def __init__(self):
        ...
        # param
        self.nFreqStim = 4 # 目标数
        self.channum = 8 # 通道数
        self.freqStim = [10,12,14,16]
        self.displayTime = 2
        self.sampleRate=250

        # FBCCA模板
        self.condY = np.zeros((self.displayTime * self.sampleRate,2,self.nFreqStim))
        ...
        for ii in range(self.nFreqStim):
            ...
            self.condY[:,:,ii] = np.concatenate([s1,s2],axis=1)
        ...
        t1 = datetime.now() 
        print("Data Initialization Time Cost: ", t1-t0)
    ...
    def data_process(self,rawdata):
        ...
        if (rawdata.shape[0] != self.displayTime * self.sampleRate or rawdata.shape[1] != self.channum):
            ...
            print("rawdata.shape[0]", rawdata.shape[0], "rawdata.shape[1]", rawdata.shape[1])
            print("self.displayTime * self.sampleRate",self.displayTime * self.sampleRate, "self.channum", self.channum)
            return -1,0
        ...
        a = 1.25
        b = 0.25
        weights = np.array([i for i in range(1,2,1)]) ** (-a) + b
        ...
        print("Data Processing Time Cost: ", t1-t0)
